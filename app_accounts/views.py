...
from .models import BasketItem, Basket, Profile, Address, Comment, Like, Question, Reply, DeliveryDate, \
    ReturningBasket, ReturningDate, ReturningItem, RefundAmount, ValidationCode
...
def add_delivery_address(request, pk):
    ...
    data = {"address": BS.address.mailing_address,
            ...
            "reciver_lat_name": BS.address.reciver_last_name}
...
def edite_returning_items(request, pk):
    ...
    return_items = [i.basket_item for i in
                    return_basket.returningitem_set.all().exclude(status='canceled')]  # $$$$$$$$$$$$$$$$$$$$$$$$$$$
            ...
            if item['delete'] == 'true':
                ...
                try:
                    ...
                    if count > itm.count or count == 0:
                        return Response(
                            {'error': 'count nabayad bish az tedad dar basket shoma bashad-va nabayad sefr bashad'})
                except:
